...
function makeTransaction(pricePerDroid, orderedQuantity, customerCredits) {
  ...
  if (customerCredits < totalPrice) {
    ...
    let changeMoney = customerCredits - totalPrice;
    message = `"You ordered ${orderedQuantity} droids, you have ${changeMoney} credits left".`;
  }
...
function isNumberNotInRange(start, end, number) {
  let isNotInRange = !(number >= start && number <= end);
...
const lastElement = courseTopic.slice(
  courseTopic.length - 1,
  courseTopic.length
);
...
function checkForName(fullName, name) {
  let result = true;
  if (name === fullName.slice(0, name.length)) {
    console.log(result);
  } else {
    console.log(!result);
  }
